...
test.describe(`Edit profile as candidate`, () => {
  ...
  test(`User should delete job experience`, async ({ page }) => {
    await page.goto('/profile/candidate/experience');

    await graphqlWaitForResponse(page, 'deleteWorkPlace', async () => {
      await page
        .getByRole('button')
    ...
    await expect(page.locator('li[class*=ProfileWorkHistory_item]')).toBeHidden();
  });
});
