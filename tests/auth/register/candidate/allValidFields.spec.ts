...
test.describe(`Register as candidate`, () => {
  test(`User should register with all valid fields`, async ({ page }) => {
    ...
    const salaryType = 'Monthly'; // 'Annual'
    ...
    const endYear = 2026;
    const previousJobAchivements = faker.lorem.sentence();
    ...
    await expect(page.getByRole('link', { name: 'Behance', exact: true })).toHaveAttribute(
      'href',
      behanceURL,
    );
